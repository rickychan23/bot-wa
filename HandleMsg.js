...
module.exports = HandleMsg = async (aruga, message) => {
    try {
        ...
        case 'speed':
        case 'ping':
            await aruga.sendText(from, `Ping! Ping! Pong!!!!\nSpeed: ${processTime(t, moment())} _Second_`)
            break
        case 'tnc':
            ...
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `Hai, Im Habib BOT. To find out the commands on this bot type ${prefix}menu`)
                      })
            } else {
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *H A B I B  B C* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *H A B I B  B C* 〙\n\n${msg}`)
            }
